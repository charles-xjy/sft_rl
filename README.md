...
- `<analysis>`：结构化证据分析，固定 3 步（定位 / 观察 / 判断）。
- `<answer>`：最终答案，简洁直接。
...
| 类型 | 说明 | 建议占比 |
|------|------|----------|
| `existence` | 是否存在某类地物 | 25% |
| `attribute` | 颜色、形状、屋顶类型、材质或纹理 | 20% |
| `location` | 某地物位于图像哪个区域 | 20% |
| `spatial_relation` | 两个地物之间的相对位置或邻接关系 | 20% |
| `counting` | 清晰可见目标的数量或估计数量 | 15% |
...
- `question_type`：existence / attribute / location / spatial_relation / counting。
