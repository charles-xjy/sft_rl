...
本项目当前聚焦于构建**单图遥感 VQA SFT 冷启动数据**。采用**纯蒸馏**方式：把"图像 + 问题"直接交给更强的教师模型（Qwen3.5-27B），它怎么回答就原样收作训练标签，**不强加任何输出格式，也不额外注入行为约束**。
...
|------|------------|-----------|------------|
| **答案格式** | 每条都强制 `<analysis>` 固定 3 步（定位/观察/判断）+ `<answer>`；后改为步数自适应 + 混 30% 直答 | 即便放宽，**固定 XML 脚手架每条都出现**仍把窄数据上的 2B 学生拽向单一模板腔，放大灾难性遗忘 | **纯蒸馏**：撤掉一切格式约束与格式校验，图+问题直接问教师、原样收下 |
...
SFT 阶段训练的是“会答、会模仿教师自然回答分布、会用自然语言答”。GRPO 阶段再重点优化“答得更准、看不清时更稳”。

因此，SFT 数据可以容忍少量噪声，但不能形成系统性坏习惯。需要重点避免：

- 答案过长，变成长篇遥感报告。
- 编造图像中不可见的证据。
- 教师答案整体过度模板化，学生只学到固定话术。
...
## GRPO 数据准备：可自动验证的遥感 VQA

SFT 之后接 GRPO，关键不在算法而在**奖励信号**。本项目 GRPO 阶段不依赖人写 reference、也不强行用 LLM 打分，而是从 [GeoChat_Instruct](https://huggingface.co/datasets/MBZUAI/GeoChat_Instruct) 里**只挑答案能被规则自动判对错的样本**，做成「闭集 / 二分类」式 VQA，让 GRPO 用**可验证奖励（exact-match）**而不是噪声很大的过程奖励。

脚本：`scratch/filter_geochat_easy_grpo.py`（输入 `GeoChat_Instruct.json`，输出到 `scratch/data/`）。

**只保留 4 类「易判分」问题**（其余一律丢弃）：

| 类型 | 判据 | 参考答案空间 |
|------|------|------|
| `scene_classification` | 题面是「classify…classes: …」，且 reference 既在候选类里、又与图片所在文件夹标签一致（双重校验，过滤错标） | 单一场景类别 |
| `binary_yesno` | 答案就是 yes / no | 是 / 否 |
| `rural_urban` | 「is it a rural or an urban area?」 | 农村 / 城市 |
| `flood_binary` | 「flooded or non flooded?」 | 洪涝 / 非洪涝 |

`binary_yesno` 再细分子类型（`presence_yesno` 存在性、`comparison_yesno` 比较、`count_equal_yesno` 计数相等、`other_yesno`），便于按子类型均衡采样、并剔除难自动判分的 `other_yesno`。

**两个关键处理**，让 GRPO 数据和 SFT 同分布、同时奖励可验证：

1. **中文化对齐**：用内置的场景标签表 / 名词 / 形容词 / 方位词词典，把英文题面和答案翻译成与 SFT 数据一致的中文口吻（如「图中有道路吗？请只回答是或否。」），并统一精简成 `{image, question, reference}` 三字段。场景题还会从全部候选里**随机抽 6 个候选**（含正确答案）拼进题面，避免候选过长、也增加判别难度。
2. **均衡采样**：按标签（yes/no、rural/urban…）做均衡，避免奖励被多数类带偏。

**产物（`scratch/data/`）**：

| 文件 | 行数 | 说明 |
|------|------|------|
| `scene_classification.jsonl` / `binary_yesno.jsonl` / `rural_urban.jsonl` / `flood_binary.jsonl` | 28362 / 40338 / 572 / 1303 | 各类型全量池 |
| `scene_classification_first30_per_folder.jsonl` | 1350 | 每个场景文件夹各取 30 张 |
| `binary_yesno_500_per_subtype.jsonl` | 1500 | 每子类型 500（按 yes/no 均衡，已排除 `other_yesno`） |
| `rural_urban_500.jsonl` / `flood_binary_500.jsonl` | 500 / 500 | 按标签均衡的采样 |
| `train_merged.jsonl` | 3850 | 上面 4 份采样合并成的 GRPO 训练集 |

`train_merged.jsonl` 配比 = 场景 1350 + 二分类 1500 + 城乡 500 + 洪涝 500。每行形如：

```json
{"image": "NWPU-RESISC45/church/church_144.jpg", "question": "请判断这张遥感图像的主要场景类别。可选类别：火力发电站、储罐区、教堂、火车站、商业区、停车场。请只输出一个类别名称。", "reference": "教堂"}
```

> 奖励侧：因为 `reference` 是闭集/二分类，GRPO 可直接做规则化 exact-match 打分。若要用过程奖励模型，`scratch/grpo_prm.py` 提供了一个对接 vLLM 推理服务的 `ClientPRM`（输出 `**Reward: x**` 数值）作为备选，但默认推荐用可验证奖励。
...
# 可调：--scene-per-folder 30 --yesno-per-subtype 500 \
#       --sample-rural-urban 500 --sample-flood-binary 500 --seed 3407
```
...
> **配比调整说明**：相比早期版本特意**压低了 `counting`（15%→5%）**——遥感计数受遮挡、分辨率、边界模糊影响极大，强行数出"17 栋"往往实际是"15~20 栋"，噪声比 existence/location 高得多。`unanswerable` 与 `ambiguous` 仍保留，用于维持问题类型覆盖，但在当前口径里不再把它们当作 SFT 阶段的“拒答能力训练”指标。
...
2. **蒸馏 prompt 就是原问题**：`ANSWER_DISTILL_PROMPT` 不再包任何任务说明或行为护栏，教师看到的就是问题文本本身。
...
> `src/validator.py` 里那套面向旧格式的强校验仍然只保留备查，不再参与当前纯蒸馏流水线。`src/health.py` 已按蒸馏形态改写，当前只关注问题分布和回答形态。
...
- **单一答案开头 Top-N 分布**：任何单一开头占比超过阈值（默认 20%）即告警——过度模板化的直接信号。
- 各题型 **answer 长度** 分布。
- **回答长度统计**是否稳定（平均值 / 中位数 / p90 / 最大值），避免整体报告化。
- 编码（乱码）自检。
...
描述、问题阶段仍保留较强约束；**答案阶段是纯蒸馏，不加行为护栏**，不规定输出格式。各阶段共同保留的底线：
