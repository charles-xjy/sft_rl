...
打开 [http://127.0.0.1:8008](http://127.0.0.1:8008)。

功能：

- 左右分栏：左侧大图、右侧问题/答案/自动质检/审批操作
- 单条样本进行 `通过 / 不通过` 判定，可写备注
- 支持 `未审批 / 全部 / 仅通过 / 仅不通过` 四种过滤
- 审批结果写入 `outputs/manual_reviews.jsonl`，**幂等覆盖**：同一样本多次审批以最后一次为准
- 进程级单例 + 全量重写，多次启动/关闭不会丢数据
- 默认仅监听 `127.0.0.1`，本机访问；如需局域网访问改 `--host 0.0.0.0`

样本主键由 `image_path | question` 组成。所以注意：

- 一旦图片路径或问题文本发生变更，旧的审批记录会"对不上"被视为未审。
- 启动时一次性加载 `--input`，**运行期间不会自动重读**——追加了新数据要 Ctrl+C 后重启。

下游消费时，把 `manual_reviews.jsonl` 与 `03_answers_sft.jsonl` 按 `id` join，仅保留 `decision == "approved"` 的样本即可得到人工审核通过的 gold/silver 集合。
