...
# 根据你的图片路径修改
base_dataset_path = "dataset/LEVIR-CD+" # 指向父目录
target_splits = ["train", "test"]       # 定义要处理的子集
for split in target_splits:
    print(f"\n开始处理数据集分级: {split}")

    # 动态构建路径
    dataset_root = os.path.join(base_dataset_path, split)
    # 获取当前时间并格式化为: 月.日.时:分
    current_time = datetime.now().strftime("%m.%d.%H:%M")
    # 动态修改输出文件名，增加 split 后缀以便区分
    output_file = f"{current_time}_LEVIR-CD+_{split}_sft.jsonl"
